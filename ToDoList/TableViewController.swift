...
//  ToDoList
//
//  Created by MacBook Pro on 2022-04-04.
//
...
class TableViewController: UITableViewController {
    ...
    @IBAction func saveTask(_ sender: UIBarButtonItem) {
        ...
        let alertController = UIAlertController(title: "New Task", message: "Please insert new task", preferredStyle: .alert)
        let saveAlert = UIAlertAction(title: "Save", style: .default) { action in
            let tf = alertController.textFields?.first
            if let newTaskTitle = tf?.text {
                self.saveNewTask(withTitle: newTaskTitle)
    ...
    private func saveNewTask(withTitle title: String) {
